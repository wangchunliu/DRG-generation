...
def add_external_tuple(simple_sbn, id_cur_sbn, cur_sbn, tuple_sbn, nodes_sbn, index):
    verb_role_list1 = ['Agent']
    verb_role_list2 = ['Patient']
    for j in range(index, len(cur_sbn) - 2, 2):
        if cur_sbn[j + 1] in verb_role_list1 and is_number(cur_sbn[j + 2]) and re.search("\-", cur_sbn[j + 2]):
            try:
                tuple_sbn.append(['Agent','Patient'])
                #tuple_sbn.append([simple_sbn[id_cur_sbn + int(cur_sbn[j + 2])][0], 'TOPIC', simple_sbn[id_cur_sbn + int(cur_sbn[j + 2])][0]])
                #tuple_sbn.append([cur_sbn[j + 1], 'TOPIC', cur_sbn[j + 1]])
            except:
                continue
        if cur_sbn[j + 1] in verb_role_list2 and is_number(cur_sbn[j + 2]) and re.search("\-", cur_sbn[j + 2]):
            try:
                tuple_sbn.append(['Patient', 'Agent'])
            except:
                continue
    return tuple_sbn, nodes_sbn
