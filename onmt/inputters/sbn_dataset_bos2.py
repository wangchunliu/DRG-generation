...
class SBNDatasetBos2(DatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, 
        ...
        super(SBNDatasetBos2, self).__init__(
    ...
    def make_SBN_examples_nfeats_tpl(text_iter, text_path, truncate,
        ...
        if text_iter is None:
            if text_path is not None:
                text_iter = SBNDatasetBos2.make_SBN_iterator_from_file(text_path)
            else:
                return (None, 0)
        ...
        examples_nfeats_iter = \
            SBNDatasetBos2.make_examples(text_iter, truncate, side, reentrancies)
    ...
    def make_examples(text_iter, truncate, side, reentrancies):
        ...
        for i, line in enumerate(text_iter):
            ...
            if side == "tgt":
                words, feats, n_feats = \
                    SBNDatasetBos2.extract_text_features(line)
                graph = None
            else:
                feats = None
                n_feats = 0
                graph = SBN_bos2.extract_SBN_features(line)
    ...
    def get_num_features(corpus_file, side):
        ...
        with codecs.open(corpus_file, "r", "utf-8") as cf:
            ...
            if side == "tgt":
                _, _, num_feats = SBNDatasetBos2.extract_text_features(f_line)
            else:
                num_feats = 0
