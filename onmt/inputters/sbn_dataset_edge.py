...
class SBNDatasetEdge(DatasetBase):
    """ Dataset for data_type=='SBN'
    ...
    def __init__(self, fields, src_examples_iter, 
        ...
        super(SBNDatasetEdge, self).__init__(
    ...
    def collapse_copy_scores(scores, batch, tgt_vocab, src_vocabs):
        ...
        for b in range(batch.batch_size):   
            ...
            src_vocab = src_vocabs[index%len(src_vocabs)] 
    ...
    @staticmethod
    def make_SBN_examples_nfeats_tpl(text_iter, text_path, truncate,
                                     side, reentrancies):
        """
        Args:
        ...
        if text_iter is None:
            if text_path is not None:
                text_iter = SBNDatasetEdge.make_SBN_iterator_from_file(text_path)
            else:
                return (None, 0)
        ...
        examples_nfeats_iter = \
            SBNDatasetEdge.make_examples(text_iter, truncate, side, reentrancies)
    ...
    def make_examples(text_iter, truncate, side, reentrancies):
        ...
        for i, line in enumerate(text_iter):
            ...
            if side == "tgt":
                words, feats, n_feats = \
                    SBNDatasetEdge.extract_text_features(line)
                graph = None
            else:
                feats = None
                n_feats = 0
                graph = SBN_edge.extract_SBN_features(line)
    ...
    def get_num_features(corpus_file, side):
        ...
        with codecs.open(corpus_file, "r", "utf-8") as cf:
            ...
            if side == "tgt":
                _, _, num_feats = SBNDatasetEdge.extract_text_features(f_line)
            else:
                num_feats = 0
...
class ShardedTextCorpusIterator(object):
    ...
    def _example_dict_iter(self, line, index):
        ...
        if self.side == 'tgt':
            ...
            example_dict = {self.side: words, self.side + "_graph": graph, "indices": index}
        if feats:
